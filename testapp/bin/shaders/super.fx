...
shared float4 fog_params; //(x=fogStart,y=fogEnd,z=(fogEnd-fogStart))
...
float4 ps_mesh(in MESH_OUTPUT In) : COLOR0
{
	...
	for(int i=0; i<LIGHTS; ++i)
	{
		...
		float light = clamp(dot(light_vec, normal),0,1) * falloff;
		if(light > 0)
		{
	...
	float3 diffuse = light_color * light_intensity;
